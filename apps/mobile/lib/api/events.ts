...
const events = resourceClient<Event>("/api/v1/events");
...
export function fetchEvent(id: string): Promise<Event> {
  return events.get<Event>(`/${encodeURIComponent(id)}`);
}
...
export function createEventBooking(
  ...
  return request<EventBooking>("/api/v1/events/bookings", { method: "POST", token, body });
}
