...
function getJson<T>(path: string, params?: Record<string, unknown>): Promise<T> {
  return request<T>(path, { errorLabel: "landing fetch failed", params });
}
...
export function fetchLandingRails(): Promise<Extract<LandingRailPayload, { type: "transport" }>>;
export function fetchLandingRails<T extends LandingRailType>(
  type: T,
): Promise<LandingRailPayload & { type: T }>;
export function fetchLandingRails(type: LandingRailType = "transport"): Promise<LandingRailPayload> {
  return getJson<LandingRailPayload>("/api/v1/landing/rails", { type });
}
