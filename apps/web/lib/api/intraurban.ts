...
export function fetchUrbanLines(): Promise<UrbanLine[]> {
  return request<UrbanLine[]>("/api/v1/intraurban/lines", { cache: "no-store" })
}

export function fetchUrbanSchedule(params: { origin?: string; dest?: string; date: string; pax?: number }): Promise<UrbanTrip[]> {
