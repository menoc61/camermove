...
import type { AgencyRoute, AgencyCategory, VehicleAmenity } from "@camermove/shared"
...
export async function fetchAgenciesList(params: AgenciesQuery = {}): Promise<{ items: AgencyListItem[]; total: number }> {
...
export async function fetchAgency(slug: string): Promise<AgencyDetail | null> {
  try {
    return await request<AgencyDetail>(`/api/v1/agencies/${encodeURIComponent(slug)}`, { cache: "no-store" })
  } catch {
    return null
  }
}