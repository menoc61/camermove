...
function departureLabel(iso?: string): string {
  if (!iso) return "Départ aujourd'hui"
...
export function Hero({ minPrice, nextDepartureAt }: HeroProps) {
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-[hsl(var(--brand-dark)/0.04)] via-background to-[hsl(var(--brand-light)/0.06)]" />
      <div
      ...
      <div
        ref={heroRef}
        className="relative mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 px-4 pb-10 pt-8 sm:px-6 lg:grid-cols-12 lg:gap-16 lg:pb-16 lg:pt-16"
      >
        {/* Content */}
        <div className="lg:col-span-6">
          ...
          >
            ...
            payez par Mobile Money et recevez votre billet instantanément.
          </motion.p>
          ...
          <motion.div
            ...
            className="relative z-10 mt-8 max-w-xl rounded-2xl border border-border/60 bg-card p-4 shadow-brand sm:p-5"
          >
            <SearchBar />
          </motion.div>
        </div>
        ...
        <div className="relative hidden lg:col-span-6 lg:block">
          <motion.div style={shouldReduce ? undefined : { opacity: bgOpacity }}>
            <motion.div
              ...
              className="hero-image relative ml-auto aspect-[4/5] w-[80%] overflow-hidden rounded-3xl shadow-brand will-change-transform"
            >
              <Image
                src="https://picsum.photos/seed/camermove-coach-highway/900/1100"
                alt="Autoroute camerounaise au coucher du soleil"
                fill
                priority
                sizes="(min-width: 1024px) 40vw, 0vw"
                className="object-cover"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[hsl(var(--brand-dark)/0.35)] via-transparent to-transparent" />
              {minPrice != null && (
                <div className="absolute bottom-4 left-4 right-4 rounded-xl border border-white/20 bg-white/90 px-4 py-3 shadow-lg backdrop-blur-sm">
                  <p className="text-xs font-medium text-muted-foreground">{departureLabel(nextDepartureAt)}</p>
            ...
            >
              <Image
                ...
                alt="Bus interurbain moderne sur la route"
                fill
                loading="lazy"
            ...
            <motion.div
              ...
              className="absolute -right-2 top-10 inline-flex items-center gap-2 rounded-full border border-border/50 bg-white px-3 py-2 shadow-md"
            >
              <MapPin className="size-4 text-[hsl(var(--brand))]" />
              <span className="text-sm font-semibold text-foreground">Yaoundé → Douala</span>
            </motion.div>
          </motion.div>
        </div>
      </div>
