...
export function prefixForKind(kind: Exclude<PayableKind, "trip">): string {
  return ENTITY_REFERENCE_PREFIX[kind]
}

export function tripPrefix(): string {
  return TRIP_REFERENCE_PREFIX
}
...
export function resolveKindByReference(reference: string): { kind: Exclude<PayableKind, "trip">; idPrefix: string } | null {
  for (const { prefix, kind } of PAYABLE_KINDS_BY_PREFIX) {
    if (reference.startsWith(prefix)) {
      const suffix = reference.slice(prefix.length)
      if (/^[A-Za-z0-9]{8}$/.test(suffix)) {
        return { kind, idPrefix: suffix.toLowerCase() }
      }
    }
  }
  return null
